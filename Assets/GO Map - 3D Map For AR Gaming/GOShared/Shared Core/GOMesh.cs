...
//			msh.RecalculateBounds();
        ...
        public Mesh ToOutlinedPolygonMesh() {

            if (secondaryMesh == null)
                return ToMesh();

            ////Set outline triangles
            Vector3[] adjustedVertices = new Vector3[secondaryMesh.vertices.Length];
            for (int i = 0; i < secondaryMesh.vertices.Length; i++)
            {
                Vector3 v = secondaryMesh.vertices[i] + (Vector3.up * 0.35f) + new Vector3(-0.6f, 0, -1f);
                adjustedVertices[i] = v;
            }

            //Sum the vertices
            Vector3[] vertsSum = vertices.Concat(adjustedVertices).ToArray();

            // Create the mesh
            Mesh msh = new Mesh();
            msh.vertices = vertsSum;
            msh.subMeshCount = 2;

            //Set fill triangles
            msh.SetTriangles(triangles, 1);
            ////Set outline triangles
            int[] adjustedTriangles = new int[secondaryMesh.triangles.Length];
            for (int i = 0; i < secondaryMesh.triangles.Length; i++) {
                int v = secondaryMesh.triangles[i] + vertices.Length;
                adjustedTriangles[i] = v;
            }

            //Debug.Log("outline triangles " + outlineTriangles.ToArray().Length);
            msh.SetTriangles(adjustedTriangles, 0);

            ////Set Uvs
            msh.uv = uv.Concat(secondaryMesh.uv).ToArray();
